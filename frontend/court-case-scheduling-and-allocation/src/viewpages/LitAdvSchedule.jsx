...
export default function LitAdvSchedule() {
  ...
  const loadData = async () => {
    const hearingsRes = await authFetchJson('/api/hearings');
  ...
  return (
    ...
    >
      ...
      <section style={{ backgroundColor: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: '10px', padding: '12px 14px', marginBottom: '16px' }}>
        <p style={{ margin: 0, color: '#1E40AF', fontSize: '13px' }}>Submit new hearing requests from the Cases page.</p>
      </section>
      ...
      {!isLoading && view === 'weekly' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '14px' }}>
          {grouped.map((col) => (
            ...
            >
              <div style={{ backgroundColor: LITIGANT_THEME.panel, padding: '10px 12px', fontWeight: 700, color: '#334155' }}>{col.day}</div>
