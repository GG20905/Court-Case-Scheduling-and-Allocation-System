...
export default function AdminPageShell({
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', sans-serif", minHeight: '100vh', backgroundColor: ADMIN_THEME.pageBg }}>
      <Navbar />

      <nav
        style={{
          backgroundColor: ADMIN_THEME.navPrimary,
          display: 'flex',
          alignItems: 'center',
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', overflowX: 'auto', flex: 1, minWidth: 0 }}>
          {navItems.map((item) => (
            <button
              ...
              style={{
                background: activeNav === item ? ADMIN_THEME.accent : 'transparent',
                color: activeNav === item ? '#fff' : ADMIN_THEME.navText,
                border: 'none',
                borderRadius: '6px',
        ...
        <div style={{ flexShrink: 0 }}>
          <ProfileMenu accentColor={ADMIN_THEME.accent} borderColor={ADMIN_THEME.border} />
        </div>
      </nav>
        ...
        <>
          ...
          <aside
            style={{
              ...
              maxWidth: '320px',
              backgroundColor: ADMIN_THEME.panel,
            ...
            {sidebarItems.map((item) => (
              <button
                ...
                style={{
                  ...
                  padding: '9px 20px',
                  fontSize: '13.5px',
                  color: activeSidebarKey === item.key ? ADMIN_THEME.accent : '#475569',
                  fontWeight: activeSidebarKey === item.key ? 600 : 400,
                  cursor: 'pointer',
                  backgroundColor: activeSidebarKey === item.key ? '#deeaf7' : 'transparent',
                  borderLeft: activeSidebarKey === item.key ? `3px solid ${ADMIN_THEME.accent}` : '3px solid transparent',
                }}
              >
                {item.label}
              </button>
